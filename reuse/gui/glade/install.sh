...
if [ -n "$SWCHOICE" ]; then
	...
	echo "XXX Creando archivo SWAP en $FSCHOICE..... XXX"
...
echo "XXX Activando partición SWAP..... XXX"
...
TOTAL=`df -m /META/ | awk '/dev/ {print $3}'`
...
while [ ! -f "/tmp/cp_end" ]; do
	TAM=`df -m $MOUNTPOINT/ | awk '/dev/ {print $3}'`
...
SPLASH=$(grep SPLASH /isolinux/conf/var.conf | cut -d '=' -f 2)
if [ $SPLASH = "Y" ]; then
	cp -f /cdrom/isolinux/cdroot/templates/initrd $MOUNTPOINT/boot/initrd-`uname -r`
fi
...
for i in `ls /tmp/cdroms` ; do 
	echo "none /$i supermount dev=/dev/$i,fs=iso9660,ro  0  0"  >> $MOUNTPOINT/etc/fstab
...
NUM=0
if [ -z "$SDA" ]; then
	echo "none /mnt/usb$NUM supermount dev=/dev/sda1,fs=vfat,sync  0  0" >> $MOUNTPOINT/etc/fstab
	mkdir $MOUNTPOINT/mnt/usb$NUM
	NUM=$[NUM+1]
fi
if [ -z "$SDB" ]; then
	echo "none /mnt/usb$NUM supermount dev=/dev/sdb1,fs=vfat,sync  0  0" >> $MOUNTPOINT/etc/fstab
	mkdir $MOUNTPOINT/mnt/usb$NUM
fi
...
if [  -f "/sbin/grub" -o -f "/sbin/grub-install" ] ; then
	...
	# Crear el menu.lst
	grub-config.sh $FSCHOICE $SPLASH > /tmp/grub.log 2>&1 &
...
echo "XXX Últimas configuraciones... XXX"
post-install.sh $USERNAME $ROOT >> /tmp/install.log 2>&1
